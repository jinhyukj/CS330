...
tid_t process_create_initd(const char *file_name)
{
	...
	//printf("new thread create for file name, %s\n", file_name);
...
static struct thread *tidSearch(tid_t fork_tid)
{
	...
	{
		void *vaddr = list_entry(e, struct thread, child_elem);
...
tid_t process_fork(const char *name, struct intr_frame *if_)
{
	...
	struct intr_frame original_if = (*(thread_current())).tf;

	memcpy(&(*(thread_current())).tf, if_, sizeof(struct intr_frame));

	tid_t fork_tid = thread_create_tf(name, PRI_DEFAULT, __do_fork, thread_current(), if_);

	struct thread *child;
	struct list_elem *e;
	child = tidSearch(fork_tid);
...
__do_fork(void *aux)
{
	...
	memcpy(&if_, (*(thread_current())).tf.R.rdx, sizeof(struct intr_frame));
	...
	for (int i = 0; i < 14; i++)
	{
		...
		if (file != NULL)
		{
			...
			{
				struct file *dupfile;
				dupfile = file_duplicate(file);
				(*(thread_current())).fd[i] = dupfile;
			}
		}
	}
	...
	sema_up(&((*(thread_current())).fork_lock));
...
int process_wait(tid_t child_tid UNUSED)
{
	...
	childThread = tidSearch(child_tid);
	...
	{
		sema_down(&((*(childThread)).wait_lock));
		returnValue = (*(childThread)).exitStatus;
		list_remove(&((*(childThread)).child_elem));
	}
...
load(const char *file_name, struct intr_frame *if_)
{
	...
	process_activate(thread_current());
	//added by jinhyuk
	if (strstr(t->name, file_name) != NULL)
